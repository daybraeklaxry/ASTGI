...
ABLATION_NAME="Sensitivity_Analysis_k"
model_name="ST-PPGN"
...
stppgn_k_neighbors_options=(2 6 12 24)
# [策略] prop_layers: 保持不变。层数过多在长序列上计算成本高且可能导致过平滑
stppgn_prop_layers_options=(2)
...
# --- 5. 随机搜索主循环 ---
for k_val in "${stppgn_k_neighbors_options[@]}"; do
  ...
  ### 修改 ###
  # --- 构建唯一的模型ID ---
  model_id="${model_name}_${dataset_name}_Sens_K${k_nn}_sl${seq_len}_pl${pred_len}_dm${dm}_L${n_prop}_dc${c_dim}_dt${t_dim}"
  echo "-----------------------------------------------------------------------"
  echo "📈 启动 K值敏感性测试 -> k_neighbors=${k_nn}，分配至空闲 GPU ${gpu_id}"
