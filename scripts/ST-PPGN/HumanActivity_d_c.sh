...
ABLATION_NAME="Sensitivity_Analysis_d_c"
model_name="ST-PPGN"
...
stppgn_channel_dims=(24 32 64 96 128)
...
# --- 5. 随机搜索主循环 ---
for dc_val in "${stppgn_channel_dims[@]}"; do
  ...
  ### 修改 ###
  # --- 构建唯一的模型ID ---
  model_id="${model_name}_${dataset_name}_Sens_Dc${c_dim}_sl${seq_len}_pl${pred_len}_K${k_nn}_L${n_prop}_D${dm}_dt${t_dim}"
  echo "-----------------------------------------------------------------------"
  echo "📈 启动 Dc 值敏感性测试 -> d_c=${c_dim}，分配至空闲 GPU ${gpu_id}"
