...
ABLATION_NAME="Sensitivity_Analysis_d_model"
model_name="ST-PPGN"
...
for dm_val in "${d_models[@]}"; do
  ...
  ### 修改 ###
  # --- 构建唯一的模型ID ---
  model_id="${model_name}_${dataset_name}_Sens_D${dm}_sl${seq_len}_pl${pred_len}_K${k_nn}_L${n_prop}_dc${c_dim}_dt${t_dim}"
  echo "-----------------------------------------------------------------------"
  echo "📈 启动 D 值敏感性测试 -> d_model=${dm}，分配至空闲 GPU ${gpu_id}"
