...
# SpatioTemporalPropagationLayer 类
class SpatioTemporalPropagationLayer(nn.Module):
    def __init__(self, d_model, d_coord, k, mlp_ratio=2.0, dropout=0.1):  # mlp_ratio 可以是float
        super().__init__()
...
class Model(nn.Module):
    '''
    ST-PPGN: Spatio-Temporal Point Process Graph Networks (FAISS-GPU Enhanced)
    '''
    ...
    def __init__(self, configs):  # configs: ExpConfigs
        ...
        self.d_c = configs.stppgn_channel_dim
        self.d_t = configs.stppgn_time_dim
        self.n_layers = configs.stppgn_prop_layers
        self.k = configs.stppgn_k_neighbors
        self.mlp_ratio = configs.stppgn_mlp_ratio
        self.w_c = configs.stppgn_channel_dist_weight
        self.w_t = 1.0
    ...
    def _find_neighbors_pytorch(self, p_query: Tensor, p_hist: Tensor, k: int,
        ...
        """
        使用纯 PyTorch 在GPU上高效地实现精确的K-NN搜索。
        该实现按批次处理，确保邻居只在同一批次样本内查找。
        """
        ...
        for i in range(B):
            ...
            # 如果历史点集为空，则无法找到邻居，用0填充 (或一个合理的占位符)
            ...
            # 2.2 分别计算各分量的平方欧氏距离矩阵
            # 矩阵形状: (num_queries_in_batch, num_hist_in_batch)
            dist_sq_c = torch.cdist(p_q_c, p_hist_c, p=2.0).pow(2)
            dist_sq_t = torch.cdist(p_q_t, p_hist_t, p=2.0).pow(2)

            # 2.3 应用权重 (self.w_c, self.w_t) 组合成最终的距离矩阵
            # 注意：我们直接在平方距离上操作，这可以避免昂贵的sqrt计算，
            # 因为它不改变距离的相对排序，所以topk结果不变。
            dist_matrix_sq = self.w_c * dist_sq_c + self.w_t * dist_sq_t
            # # 1. 计算批次内所有查询点到所有历史点的欧氏距离矩阵
            # # 矩阵形状: (num_queries_in_batch, num_hist_in_batch)
            # dist_matrix = torch.cdist(p_q_batch, p_hist_batch, p=2.0)

            k_actual = min(k, num_hist_in_batch)

            # 2. 查找每行中 k_actual 个最小距离的索引
            # `torch.topk` 默认找最大值, 设置 largest=False 来找最小值
            # 返回值: (距离, 局部索引)
